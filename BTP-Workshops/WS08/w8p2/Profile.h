...
namespace sdds {
	...
	struct Profile {
		...
		// TODO: add a function here to validate the address  ANA NAN
		void validateAddress()
		{
			if (m_address.number <= 0 or m_address.postal_code.length() != Address_postal_code_length or
				!std::isalpha(m_address.postal_code[0]) or
				!std::isdigit(m_address.postal_code[1]) or
				!std::isalpha(m_address.postal_code[2]) or
				(m_address.postal_code[3] != ' ') or
				!std::isdigit(m_address.postal_code[4]) or
				!std::isalpha(m_address.postal_code[5]) or
