...
namespace sdds
{
    ...
    AirportLog::AirportLog(string filename) : AirportLog()
    {
        //todo: 
        /*
            A one argument constructor that receives a filename as a parameter (review the file airport_data.csv to understand its format) 
            from which we will populate the current object. This function will:
            Read the file to count the number of airports in the file (note the heading line in the file is not describing an actual 
            airport so do not count that line)
    ...
    AirportLog& AirportLog::operator=(const AirportLog &that)
    {
        if (this != &that) 
        {
            ...
            Airport* anotherAirport = new Airport[that.capacity];

            for (size_t i = 0; i < capacity; ++i) 
            {
                anotherAirport[i] = that.airport[i];
            }
            delete[] airport;
            this->airport = anotherAirport;
        }
        return *this;
    }
    ...
    {
        /*
        A modifier that receives a constant reference to an Airport object and returns nothing. 
        addAirport will attempt to append the parameter to the dynamic array of airports held by 
        the current object. Resize the array appropriately to accommodate the new object and avoid
        memory leaks.  
        */
        if (!that.m_code.empty())
        {
    ...
    {
        /*
        A query that receives two C-style strings as paramenters representing the state name and 
        the country name and returns an AirportLog object. The returned object will contain all of
        the airports in that state and country.
        */
        ...
        for (size_t i = 0; i < capacity; i++) 
        {
            if ((airport[i].m_state == state) and (airport[i].m_country == country)) 
            {
                that.addAirport(airport[i]);
            }
        }
        return that;
    }
    ...
    {
        /*
        A casting operator that will convert the current object to a size_t value. 
